<?php $__env->startSection('title', 'Buscador de Recursos - CorRol'); ?>

<?php $__env->startSection('content'); ?>
<div class="space-y-8">
    
    <!-- Buscador Simple y Avanzado -->
    <form action="<?php echo e(route('resources.index')); ?>" method="GET" class="bg-slate-800 p-6 rounded-xl border border-slate-700 space-y-4 shadow-xl">
        <div class="flex flex-col sm:flex-row gap-3">
            <input type="text" name="q" value="<?php echo e($filters['q'] ?? ''); ?>" placeholder="Buscar por título, descripción, juego, autor..."
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-2 text-sm">
            <select name="game" class="bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-slate-300">
                ...
                <?php $__currentLoopData = $games; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $g): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($g); ?>" <?php echo e(($filters['game'] ?? '') === $g ? 'selected' : ''); ?>><?php echo e($g); ?></option>
            ...
            <select name="campaign" class="bg-slate-900 border border-slate-700 rounded-lg p-2 text-slate-300">
                ...
                <?php $__currentLoopData = $campaigns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($c); ?>" <?php echo e(($filters['campaign'] ?? '') === $c ? 'selected' : ''); ?>><?php echo e($c); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
            ...
            <select name="author" class="bg-slate-900 border border-slate-700 rounded-lg p-2 text-slate-300">
                ...
                <?php $__currentLoopData = $authors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($a); ?>" <?php echo e(($filters['author'] ?? '') === $a ? 'selected' : ''); ?>><?php echo e($a); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>

            <a href="<?php echo e(route('resources.index')); ?>" class="text-slate-300 hover:text-slate-200 text-center self-center">
                Limpiar Filtros
            </a>
        </div>
    </form>
                    ...
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <span class="text-xs text-slate-300">
                            <strong class="text-slate-200">
                                <?php echo e($resource->game ?? 'General'); ?><br /><?php echo e($resource->campaign ?? ''); ?><br /><?php echo e($resource->author ?? 'Anónimo'); ?>

                            </strong>
                        </span>
                    </div>
                    <h2 class="text-xl font-bold text-slate-100 mb-1">
                        <a href="<?php echo e(route('resources.show', $resource->id)); ?>" class="hover:text-indigo-400">
                            <?php echo e($resource->title); ?>

                        </a>
                    </h2>
                    <p class="text-slate-300 text-sm line-clamp-2 mb-4"><?php echo e($resource->description ?? 'Sin descripción'); ?></p>
                </div>

                <div class="pt-3 border-t border-slate-700/60 flex justify-between items-center text-xs text-slate-400">
                    <span>Subido por: <strong class="text-slate-300"><?php echo e($resource->user->name ?? 'Anónimo'); ?></strong></span>
                    <a href="<?php echo e(route('resources.show', $resource->id)); ?>" class="text-indigo-400 hover:underline font-bold">
                        Ver Recurso →
                    </a>
                </div>
            </div>
