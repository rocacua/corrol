    ...
    <form method="GET" action="<?php echo e(route('resources.index')); ?>" id="advanced-search" class="space-y-6">
        ...
        <div class="flex flex-wrap justify-between items-center gap-4 bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-xl">
            <div>
                ...
                <?php if($resources->total() > 0): ?>
                    <?php
                        $totalFavs = $resources->total_favorited ?? $resources->filter(fn($r) => ($r->favorited_by_count ?? 0) > 0)->count();
                    ?>
                    <p class="text-xs text-slate-400 mt-1">
                        Encontrado <strong class="text-indigo-300"><?php echo e($resources->total()); ?></strong> recursos.
                        <?php if($totalFavs > 0): ?>
                            <strong class="text-amber-400"><?php echo e($totalFavs); ?></strong> favoritos.
                        <?php endif; ?>
                    </p>
                <?php else: ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="sort" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-xs text-slate-100">
                        ...
                        <option value="affinity" <?php echo e(($filters['sort'] ?? '') === 'affinity' ? 'selected' : ''); ?>>Afinidad (Favoritos y Propios primero)</option>
                    ...
                    <div>
                        <div class="flex justify-between items-start gap-2 mb-2">
                            <span class="px-2.5 py-0.5 bg-indigo-600/30 text-indigo-300 border border-indigo-500/30 text-[10px] font-bold rounded-full uppercase shrink-0">
                                <?php echo e($resource->type == 'file' ? $resource->resourceable->file_type : $resource->type); ?>

                            </span>
                            <span class="text-xs <?php echo e($resource->privacy === 'private' ? 'text-amber-400' : 'text-emerald-400'); ?> shrink-0">
                                ...
                                <?php if(($resource->favorited_by_count ?? 0) > 0): ?>
                                    <br /><span class="text-[10px] text-amber-400 font-bold" title="Favorito de <?php echo e($resource->favorited_by_count); ?> usuarios">⭐ <?php echo e($resource->favorited_by_count); ?></span>
                                <?php endif; ?>
                            </span>
                            ...
                            <span class="text-xs text-slate-300 min-w-0 max-w-[150px] sm:max-w-[180px] md:max-w-[200px] text-right">
                                <strong class="text-slate-200 block min-w-0">
                                    <span class="block truncate" title="🎲 <?php echo e($resource->game ?? 'General'); ?>">🎲 <?php echo e($resource->game ?? 'General'); ?></span>
                                    <?php if($resource->campaign): ?>
                                        <span class="block truncate mt-0.5" title="📜 <?php echo e($resource->campaign); ?>">📜 <?php echo e($resource->campaign); ?></span>
                                    <?php endif; ?>
                                    <span class="block truncate mt-0.5" title="✍️ <?php echo e($resource->author ?? 'Anónimo'); ?>">✍️ <?php echo e($resource->author ?? 'Anónimo'); ?></span>
